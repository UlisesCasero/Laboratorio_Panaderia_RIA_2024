...
export class OrdenCompraService {
  ...
  getOrdenesCompra(): Observable<OrdenCompra[]> {
    ...
    return this.http.get<OrdenCompra[]>(this.apiUrl, httpOptions).pipe(
      catchError((error) => {
  ...
  getOrdenCompraByCienteId(id: number): Observable<OrdenCompra[]> {
    ...
    return this.http.get<OrdenCompra[]>(url, httpOptions).pipe(
      catchError((error) => {
        return throwError(error);
      })
    );
  ...
  getOrdenCompraByPanaderoId(): Observable<OrdenVista[]> {
    ...
    return this.http.get<OrdenVista[]>(url, httpOptions).pipe(
      catchError((error) => {
        return throwError(error);
      })
    );
  ...
  postOrdenCompra(orden: OrdenCompra): Observable<OrdenCompra> {
    ...
    return this.http.post<OrdenCompra>(this.apiUrl, orden, httpOptions).pipe(
      catchError((error) => {
        return throwError(error);
      })
    );
  ...
  filtrarInsumosPorFechaYEstadoAdmin(
    fechaInicio: string,
    fechaFin: string,
    estado: string
  ): InsumoTotal[] {
    ...
    this.listaTotalInsumosAdmin.forEach((orden) => {
      if (
        (fechaInicio === '' || orden.fecha_entrega >= fechaInicio) &&
        (fechaFin === '' || orden.fecha_entrega <= fechaFin) &&
        (estado === '' || orden.estadoOrden === estado)
      ) {
        orden.insumos.forEach((insumo) => {
          const existeInsumo = insumosFiltrados.find(
            (i) => i.insumoNombre === insumo.insumoNombre
          );
          if (existeInsumo) {
